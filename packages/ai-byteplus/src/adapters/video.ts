 ...
 *
 * Unlike images (and unlike chat multimodal video), Seedance **does not**
 * accept inline data URIs for video or audio references — Ark answers
 * `400 InvalidParameter: reference_video must be provided as a web url`
 * (and the same for audio). Host the file somewhere public (or register it
 * in the Ark asset library as `asset://…`) and pass `source: { type: 'url' }`.
 *
...
function mediaPartToRemoteUrl(
  ...
  // source.type === 'url' here; still reject non-web forms that would only
  // ever 400 (blob:, relative paths, bare filenames).
  if (
